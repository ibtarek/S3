...
const genTopo = require('./lib/utilities/topology').default;

function createTopoFile(topoMD) {
    // if topology and raw topology does not exist, create it
    // otherwise, import it
    const topoFile = `${__dirname}/${topoMD.name}.json`;
    let topology;
    let createTopo = true;
    try {
        ...
        try {
            ...
            logger.warn('Failed to import topology', { topoFile, error });
        }
    } catch (error) {
        logger.debug('Not found topology file. Check raw topology');
        const rawTopoFile = `${__dirname}/${topoMD.name}.raw.json`;
        try {
            fs.statSync(rawTopoFile);
            // import raw topology
            try {
                const topo = JSON.parse(fs.readFileSync(rawTopoFile));
                // update weight distribution
                topology = genTopo.update(topo, topoMD);
            } catch (error) {
                logger.warn('Failed to import raw topology',
                    { rawTopoFile, error });
            }
        } catch (err) {
    ...
    if (createTopo) {
        ...
        fs.writeFileSync(topoFile, JSON.stringify(topology, null, 4), 'utf8');
    }
