    ...
    startup() {
        ...
        fs.readdir( pluginDir , ( err , files ) => {
            if ( err ){
                ...
                //loads all js files in folder then calls the plugin function
                files
                .filter( f => f.lastIndexOf('.js') === f.length - 3 )
                .map( f => pluginDir+'/'+f )
                .forEach( moduleName => {
                    try{ 
                        require(moduleName)(self)
                    } catch ( error ){
                        logger.error("Module %s didn't load", moduleName, error);
                }})
                //then starts the server
                self.server.listen(_config.port);
            }
        } )
