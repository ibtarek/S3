...
export default {
    ...
    dataStore(objectMetadata, objectContext,
        ...
        data.put(cipherBundle, dataStream, size, objectContext, log,
                 (err, dataRetrievalInfo, storedHashStream , returnedHashStream , storedSize) => {
                     if (err) {
                         log.error('error in datastore', { error: err });
                         return cb(err);
                     }

                     log.info('Returned to services from backend ' , dataRetrievalInfo)
                     ...
                     let barrier = 2;
                     let returndedHash , storedHash;


                      function hashingDone(){
                        barrier--;
                        log.info('barrier is now',barrier);
                        if ( barrier <= 0 ){
                          returnedHashStream.removeAllListeners('hashed');
                          storedHashStream.removeAllListeners('hashed');

                          log.info('Hashes stored = %s returned = %s',storedHash,returndedHash)

                          return cb(null, objectMetadata, dataRetrievalInfo, storedHash , returndedHash , storedSize );
                         }
                        
                      }

                     if ( ! returnedHashStream.completedHash ){
                        returnedHashStream.on('hashed', () => {
                           log.info('returned hash ready');
                           returndedHash = returnedHashStream.completedHash
                           hashingDone()
                       }); 
                     } else {
                        returndedHash = returnedHashStream.completedHash
                     ...
                     if ( ! storedHashStream.completedHash ){
                      storedHashStream.on('hashed', () => {
                         log.info('stored hash ready');
                         storedHash = storedHashStream.completedHash
                         hashingDone()
                     });
                     } else {
                        storedHash = storedHashStream.completedHash
