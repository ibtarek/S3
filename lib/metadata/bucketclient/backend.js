...
class BucketClientInterface {
    constructor() {
        ...
        this.metadataVersion = 1; // default version 6.3.0 or earlier
    ...
    init() {
        ...
        this.client.request('GET', beginPath, logger, null, null,
            (err, data) => {
                if (!err) {
                    try {
                        const info = JSON.parse(data);
                        this.metadataVersion = Number(info.metadataVersion);
                        this.extendedVersion = this.metadataVersion > 1;
                    } catch (error) {
    ...
    createBucket(bucketName, bucketMD, log, cb) {
        if (this.gotMetadataInfo === false) {
            // not really DBAPINotReady, but cannot make request anyway
            return cb(errors.DBAPINotReady);
        }
