...
function _prepMetadata(sourceObjMD, headers, sourceIsDestination, authInfo,
    ...
    // If COPY, pull AWS-compatible headers from source object
    // Otherwise, pull AWS-compative headers from request headers
    const compatHeadersSource = whichMetadata === 'COPY' ?
        sourceObjMD : headers;
    ...
    const storeMetadataParams = {
        ...
        cacheControl: compatHeadersSource['cache-control'],
        contentDisposition: compatHeadersSource['content-disposition'],
        expires: compatHeadersSource.expires,
        overrideMetadata,
        lastModifiedDate: new Date().toJSON(),
    };
    // Remove V4 streaming value 'aws-chunked' if present in content-encoding to
    // be compatible with AWS behavior
    if (compatHeadersSource['content-encoding']) {
        storeMetadataParams.contentEncoding =
            compatHeadersSource['content-encoding']
            .replace(/aws-chunked,?/, '');
    }
